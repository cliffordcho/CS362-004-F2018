...
//int z = 0; //This is the correct one
int z; //this is the incorrect one
int drawntreasure = 0;
int cardDrawn;
...
int playCouncil_Room(struct gameState *state, int handPos) {
  ...
  //discardCard(handPos, currentPlayer, state, 0); //incorrect, need to discard
...
int playSmithy(struct gameState *state, int handPos) {
  ...
  int currentPlayer = whoseTurn(state);
  //+3 Cards
  for (i = 0; i <= 3; i++) //incorrect: change <= to <
  {
    drawCard(currentPlayer, state);
  }
